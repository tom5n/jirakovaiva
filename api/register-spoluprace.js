...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (dbError) {
      ...
      return res.status(500).json({ error: 'Nepodařilo se uložit žádost o spolupráci do databáze' });
    }

    await resend.emails.send({
      from: 'Spolupráce <info@jirakovaiva.cz>',
      to: 'jirakovaiva@seznam.cz',
      subject: 'Nová žádost o spolupráci FARMASI',
      html: `
        <!DOCTYPE html>
        <html lang="cs">
        <head>
          <meta charset="UTF-8">
          <title>Nová žádost o spolupráci</title>
        </head>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; border-radius:16px; box-shadow:0 2px 8px #0001; overflow:hidden; outline:2px solid #21435F; outline-offset:0;">
                  <tr>
                    <td style="background:#FFF5F0; padding:24px 32px 12px 32px; text-align:center;">
                      <h2 style="color:#21435F; font-family:'Montserrat',Arial,sans-serif; margin:0; font-size:22px; text-transform:uppercase;">
                        NOVÁ ŽÁDOST O SPOLUPRÁCI
                      </h2>
                    </td>
                  </tr>
                  <tr>
                    <td style="background:#fff; padding:24px 32px 32px 32px; color:#333; font-family:Arial,sans-serif; font-size:16px;">
                      <div style="background:#FFE3D6; border-radius:8px; padding:16px; color:#21435F; font-size:16px; margin-bottom:24px;">
                        Nová žádost o spolupráci s FARMASI z formuláře na webu. Vyplněné údaje:
                      </div>
                      ...
                      <hr style="border:none; border-top:1px solid #FFD1C1; margin:12px 0;">
                      <p style="margin:0 0 8px 0;"><strong>ID žádosti:</strong> <span style="color:#21435F;">${dbData[0].id}</span></p>
                    </td>
                  </tr>
                  <tr>
                    <td style="background:#FFD1C1; color:#21435F; text-align:center; font-size:14px; padding:16px 32px;">
                      Odesláno z www.jirakovaiva.cz/spoluprace
                    </td>
                  </tr>
                </table>
              </td>
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: 'Děkujeme za zájem o spolupráci s FARMASI',
      html: `
        <!DOCTYPE html>
        <html lang="cs">
        <head>
          <meta charset="UTF-8">
          <title>Děkujeme za zájem o spolupráci</title>
        </head>
        <body style="margin:0; padding:0;">
          <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 0;">
            <tr>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; border-radius:16px; box-shadow:0 2px 8px #0001; overflow:hidden; outline:2px solid #21435F; outline-offset:0;">
                  <tr>
                    <td style="background:#FFF5F0; padding:24px 32px 32px 32px; color:#333; font-family:Arial,sans-serif; font-size:16px;">
                      ...
                      <p style="margin:0 0 16px 0;">děkuji za Váš zájem o spolupráci s FARMASI pod mým vedením.</p>
                      <p style="margin:0 0 16px 0;">Vaši žádost jsem přijala a ozvu se Vám co nejdříve, abychom společně probrali možnosti spolupráce, podporu i další kroky.</p>
                      <p style="margin:0 0 16px 0;">Prosím o chvilku strpení, všechny žádosti vyřizuji postupně.</p>
                      <p style="margin:0 0 16px 0;">Těším se na naši spolupráci!</p>
    ...
    return res.status(200).json({
      ...
      message: 'Žádost o spolupráci byla úspěšně uložena a odeslána'
    });
  } catch (error) {
    console.error('Error:', error);
    return res.status(500).json({ error: 'Něco se pokazilo při zpracování žádosti o spolupráci' });
  }
}

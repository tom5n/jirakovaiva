...
import { Wallet, Clock, Home, Handshake, Gift, Plane } from "lucide-react";
import SeoHead from '../components/SeoHead';

const Spoluprace = () => {
  const discountBenefits = [
    {
      text: "Nadstandardní příjem",
      icon: <Wallet size={16} />
    },
    {
      text: "Flexibilní pracovní doba",
      icon: <Clock size={16} />
    },
    {
      text: "Práce online z domova",
      icon: <Home size={16} />
    },
    {
      text: "Moje osobní podpora a know-how",
      icon: <Handshake size={16} />
    },
    {
      text: "Exkluzivní bonusy a finanční prémie",
      icon: <Gift size={16} />
    },
    {
      text: "Cestování zdarma",
      icon: <Plane size={16} />
    },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/register-spoluprace', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        window.alert('Něco se pokazilo při odesílání žádosti. Zkuste to prosím znovu.');
      }
    } catch (err) {
      window.alert('Něco se pokazilo při odesílání žádosti. Zkuste to prosím znovu.');
    }
  ...
  return (
    <>
      <SeoHead
        title="Spolupráce | FARMASI | Ivana Jiráková"
        description="Začněte spolupráci s FARMASI pod vedením Ivany Jirákové. Vyplňte formulář a ozveme se vám."
        url="https://www.jirakovaiva.cz/spoluprace"
      />
          ...
          <section className="section-padding bg-white relative z-10">
            <div className="container mx-auto">
              <div className="text-center mb-12">
                ...
                Buduj svou kariéru s FARMASI a staň se pánem svého času
                </h1>
                <p className="text-lg text-gray-700 max-w-3xl mx-auto animate-fade-in">
                Toužíš po finanční nezávislosti a časové svobodě? Zanech mi na sebe kontakt a společně se podíváme na možnosti, které budou přímo tobě ušité na míru.
                </p>
              </div>

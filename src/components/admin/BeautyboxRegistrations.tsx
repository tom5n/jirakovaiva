...
export default function BeautyboxRegistrations({ count }: { count: number }) {
  ...
  const [allRegistrations, setAllRegistrations] = useState<BeautyboxRegistration[]>([]);
      ...
      <div className="hidden md:block">
        ...
        <div className="space-y-2">
          {filteredRegistrations.length === 0 && (
            <div className="text-gray-500 text-center py-8">
              {searchQuery ? 'Nenalezeny žádné registrace.' : 'Žádné registrace.'}
            </div>
          )}
        ...
        {searchQuery && filteredTotalCount > 0 && (
          <div className="flex justify-center items-center mt-4">
            <span className="text-sm text-gray-600">
              Nalezeno {filteredTotalCount} {filteredTotalCount === 1 ? 'registrace' : filteredTotalCount < 5 ? 'registrace' : 'registrací'}
            </span>
          </div>
        )}
      </div>

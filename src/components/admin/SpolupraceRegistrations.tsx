...
interface SpolupraceRegistration {
  id: string;
  name: string;
  surname: string;
  email: string;
...
export default function SpolupraceRegistrations({ count }: { count: number }) {
  const [registrations, setRegistrations] = useState<SpolupraceRegistration[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedRegistration, setSelectedRegistration] = useState<SpolupraceRegistration | null>(null);
  const [toast, setToast] = useState<{text: string, type: 'success'|'error'}|null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalCount, setTotalCount] = useState(0);
  const [allRegistrations, setAllRegistrations] = useState<SpolupraceRegistration[]>([]);
  ...
  const fetchRegistrations = async (page: number = 1) => {
    ...
    if (error) {
      ...
      showToast('Chyba při načítání žádostí.', 'error');
    } else {
  ...
  useEffect(() => {
    // Při změně vyhledávání resetujeme na první stránku
    if (searchQuery) {
      setCurrentPage(1);
    }
  }, [searchQuery]);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Opravdu chcete smazat tuto žádost?')) {
      ...
      if (error) {
        ...
        showToast('Chyba při mazání žádosti.', 'error');
      } else {
        showToast('Žádost byla smazána.');
        fetchRegistrations(currentPage);
        fetchAllRegistrations();
      }
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `spoluprace_registrations_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  // Filtrování registrací podle vyhledávacího dotazu
    ...
    return (
      <div className="space-y-2">
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-600">Načítání žádostí...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="hidden md:block">
        ...
        <div className="space-y-2">
          {filteredRegistrations.length === 0 && (
            <div className="text-gray-500 text-center py-8">
              {searchQuery ? 'Nenalezeny žádné žádosti.' : 'Žádné žádosti.'}
            </div>
          )}
        ...
        {searchQuery && filteredTotalCount > 0 && (
          <div className="flex justify-center items-center mt-4">
            <span className="text-sm text-gray-600">
              Nalezeno {filteredTotalCount} {filteredTotalCount === 1 ? 'žádost' : filteredTotalCount < 5 ? 'žádosti' : 'žádostí'}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 z-[101] max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-[#21435F]">Detail žádosti</h3>
              <button
